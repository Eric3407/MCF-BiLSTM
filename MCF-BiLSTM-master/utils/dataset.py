...
class ROIGenerator:
    def __init__(self, mode='classification', length=224,
    ...
    def __call__(self):
        ...
        if label == 0:  # 0 stands for noise
            ...
            if shape == 'linear':
                ...
                end = np.random.randint(low=int(a + 1.9 * sigma), high=int(a + 2.6 * sigma))
...
class ROIDataset(Dataset):
    def __init__(self, path, key, mode='classification', length=224,
